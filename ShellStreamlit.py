...
##########################################################################
with DataSet:
    with st.spinner('⚙️ Working Progress'):
        option = st.sidebar.selectbox('Please Select Maximum number of training steps',("150 Step","100 Step")) 
    ...
    with Dataframes_model.container():
        ...
        if LSTM_checkbox:
            if option == "150 Step":
                ...
                st.dataframe(LSTM_Model150().style.highlight_max("Net Cashflow from Operations"),use_container_width=True)
                @st.cache_data
                def convert_df(df):
                    return df.to_csv().encode('utf-8')
                model_csv_df150 = LSTM_Model150()
                model_csv_150 = convert_df(model_csv_df150)
                st.download_button('Download the file',model_csv_150,file_name ="Model150.csv")
            else:
                st.title("Forecasting Net Cashflow from Operations")
                st.dataframe(LSTM_Model100().style.highlight_max("Net Cashflow from Operations"),use_container_width=True)
                @st.cache_data
                def convert_df(df):
                    return df.to_csv().encode('utf-8')
                model_csv_df100 = LSTM_Model100()
                model_csv_100 = convert_df(model_csv_df100)
                st.download_button('Download the file',model_csv_100,file_name ="Model100.csv")
